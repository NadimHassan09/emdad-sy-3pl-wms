...
export function useUpdateDetector() {
  ...
  const checkForUpdate = useCallback(async (): Promise<boolean> => {
    ...
    try {
      // Cache-busting query parameter + no-store header to bypass browser and proxy caches
      ...
      // Server might be temporarily restarting or offline; quietly ignore.
      return false;
    } finally {
  ...
  useEffect(() => {
    ...
    // 2. Listen to internal update signals (e.g. dynamic chunk loading failure)
  ...
  const refreshApp = useCallback(() => {
    // Hard reload to pull the new index.html and fresh bundles.
    // Auth tokens in sessionStorage / localStorage and cookies remain intact.
