...
import {
  OutboundApi,
  type OutboundImportExecuteResult,
  type OutboundImportValidateResult,
} from '../../api/outbound';
...
export function OutboundOrdersImportModal({ open, onClose, onImported }: Props) {
  const toast = useToast();
  const { t } = useWmsTranslation();
  const inputRef = useRef<HTMLInputElement>(null);
  const [file, setFile] = useState<File | null>(null);
  const [validation, setValidation] = useState<OutboundImportValidateResult | null>(null);
  const [result, setResult] = useState<OutboundImportExecuteResult | null>(null);
  ...
  const validateMut = useMutation({
    mutationFn: (f: File) => OutboundApi.validateImport(f),
    onSuccess: (data) => {
      setValidation(data);
      setResult(null);
    },
  ...
  const importMut = useMutation({
    mutationFn: (f: File) => OutboundApi.importOrders(f),
    onSuccess: (data) => {
      setResult(data);
      toast.success(
        t([
  ...
  return (
    <Modal
      open={open}
      onClose={close}
      title={t(['Import outbound orders', 'استيراد طلبات الصادر'])}
    ...
    >
      <div className="space-y-4 text-sm text-text-body">
        <p>
          {t([
            'Upload a CSV to create outbound orders in Draft (workers mode, manual shipping). Orders are not confirmed and no allocate, tasks, or carrier shipment is created.',
            'ارفع ملف CSV لإنشاء طلبات صادر بحالة مسودة (وضع العمال، شحن يدوي). لن يتم التأكيد ولن يُنشأ تخصيص أو مهام أو شحنة ناقلة.',
          ])}
        </p>
        ...
        <div className="flex flex-wrap gap-2">
          <Button
            ...
            size="sm"
            onClick={() => {
              void OutboundApi.downloadImportTemplate().catch((err: Error) => toast.error(err.message));
            }}
          >
          ...
          <div className="rounded-lg border border-border bg-surface-sunken p-3">
            ...
            {validation.errors.length > 0 ? (
              <Button
                ...
                size="sm"
                onClick={() =>
                  downloadText('outbound-import-validation-errors.csv', errorsToCsv(validation.errors))
                }
              >
          ...
          <div className="rounded-lg border border-status-success-border bg-status-success-bg p-3 text-status-success-fg">
            ...
            {result.errors.length > 0 ? (
              <Button
                ...
                size="sm"
                onClick={() =>
                  downloadText('outbound-import-errors.csv', errorsToCsv(result.errors))
                }
              >
                {t(['Download errors CSV', 'تنزيل أخطاء CSV'])}
              </Button>
