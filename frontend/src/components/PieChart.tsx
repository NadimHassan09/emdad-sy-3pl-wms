...
export function PieChart({ title, slices, size = 200 }: PieChartProps) {
  ...
  return (
    <div className="rounded-lg border border-border bg-surface-panel p-4 shadow-sm">
              ...
              return positive.map((sl) => {
                ...
                return (
                  <path
                    ...
                    d={d}
                    fill={sl.color}
                    stroke="var(--surface-panel, #ffffff)"
                    strokeWidth="1"
                  >
      ...
      {total === 0 ? (
        <p className="mt-3 text-center text-xs text-text-muted">No open orders in this view.</p>
      ) : null}
    </div>
  );
}
