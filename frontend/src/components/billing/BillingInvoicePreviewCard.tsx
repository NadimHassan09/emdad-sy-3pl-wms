...
export function BillingInvoicePreviewCard({
  ...
  return (
    <section className="rounded-lg border border-border bg-surface-card p-4 shadow-sm">
      <h3 className="text-sm font-semibold text-text-strong">Current cycle preview</h3>
      <p className="mt-1 text-xs text-text-muted">
        Live draft totals — not a finalized invoice. Updates when operations complete.
      </p>

      {query.isPending ? <p className="mt-4 text-sm text-text-muted">Loading preview…</p> : null}
      {query.isError ? <p className="mt-4 text-sm text-status-error-fg">Could not load preview.</p> : null}

      {data ? (
        <dl className="mt-4 grid gap-3 sm:grid-cols-2">
          <div>
            <dt className="text-xs text-text-muted">Days remaining</dt>
            <dd className="text-sm font-semibold">{data.cycle.daysRemaining}</dd>
          </div>
          <div>
            <dt className="text-xs text-text-muted">Used / allocated CBM</dt>
      ...
      {preview ? (
          ...
          <ul className="mt-3 divide-y divide-border-subtle text-sm">
            {preview.lines.map((line) => (
              <li key={line.id} className="flex justify-between py-1.5">
                <span className="text-text-body">{line.type.replace(/_/g, ' ')}</span>
        ...
        <p className="mt-4 text-sm text-text-muted">No draft invoice for the current cycle.</p>
      )}
    </section>
  );
}
