...
export function OrderManualChargesSection({ referenceType, referenceId, canEdit }: Props) {
  ...
  return (
    <section className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
      <h3 className="text-sm font-semibold text-slate-900">Manual charges (VAS)</h3>
      <p className="mt-1 text-xs text-slate-500">
        These charges are included automatically in the client&apos;s billing cycle invoice.
      </p>

      {chargesQuery.isPending ? <p className="mt-3 text-sm text-slate-500">Loading charges…</p> : null}
      ...
      {charges.length ? (
        ...
        <p className="mt-3 text-sm text-slate-500">No manual charges on this order.</p>
      )}
