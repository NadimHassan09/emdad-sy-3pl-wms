...
export const TASK_PRINT_PAGE_STYLES = `
...
export function taskPrintNotesBlock(notes: string): string {
  return notes.trim()
    ? `<p class="notes">${esc(notes.trim()).replace(/\n/g, '<br/>')}</p>`
    : '<p class="muted">—</p>';
}

export function openTaskPrintHtml(documentTitle: string, bodyInnerHtml: string): boolean {
