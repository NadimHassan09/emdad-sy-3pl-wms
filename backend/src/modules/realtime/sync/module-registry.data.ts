/**
 * Module Registry — data only (Architecture 2.2 §11).
 * Domain services must never hard-code module lists; they publish(mutationId).
 */
import type { AppModuleId } from './app-modules';

export type ModuleRegistryRow = {
  /** Client Domain modules (tenant-scoped). Empty = no client emit. */
  client: AppModuleId[];
  /** Admin Domain modules (global). Empty = no admin emit. */
  admin: AppModuleId[];
};

/** Mutation id → affected modules per sync domain. */
export const MODULE_REGISTRY: Record<string, ModuleRegistryRow> = {
  InboundCreated: {
  ...
  /** Dashboard fan-out from scheduler — admin only */
  DashboardUpdated: {
    client: ['dashboard'],
    admin: ['dashboard'],
  },
...
export function resolveRegistry(mutationId: string): ModuleRegistryRow {
  const row = MODULE_REGISTRY[mutationId];
  if (!row) {
    return { client: [], admin: [] };
  }
  return row;
