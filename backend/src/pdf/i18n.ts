...
export function normalizeLang(raw?: string | null): DocLang {
  return (raw ?? '').toLowerCase().startsWith('ar') ? 'ar' : 'en';
}

/** Full label dictionary for every string rendered in the GRN / DN documents. */
const DICT = {
  // Generic
  dir: { en: 'ltr', ar: 'rtl' },
...
export function makeTranslator(lang: DocLang) {
...
/** Pre-resolve every label for a language (used as the `L` object in templates). */
export function buildLabels(lang: DocLang): Record<LabelKey, string> {
  const out = {} as Record<LabelKey, string>;
  (Object.keys(DICT) as LabelKey[]).forEach((key) => {
    out[key] = DICT[key][lang];
  });
  return out;
}

const LOCALE: Record<DocLang, string> = { en: 'en-GB', ar: 'ar-SY' };

export function formatDocDate(value: Date | string | null | undefined, lang: DocLang): string {
  if (!value) return '—';
...
export function formatDocDateTime(value: Date | string | null | undefined, lang: DocLang): string {
  if (!value) return '—';
  const d = typeof value === 'string' ? new Date(value) : value;
  if (Number.isNaN(d.getTime())) return '—';
  return new Intl.DateTimeFormat(LOCALE[lang], {
...
export function formatQty(value: string | number | null | undefined, lang: DocLang): string {
