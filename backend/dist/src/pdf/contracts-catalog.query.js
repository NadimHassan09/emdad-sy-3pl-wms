...
exports.listContractCatalogSlots = listContractCatalogSlots;
const client_1 = require("@prisma/client");
const FULL_UUID = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;
function includesReceiving(type, referenceType) {
    if (type === client_1.DocumentType.delivery_note)
        return false;
    if (referenceType === 'outbound_order')
        return false;
    return true;
}
function includesDispatch(type, referenceType) {
    if (type === client_1.DocumentType.grn)
        return false;
    if (referenceType === 'inbound_order')
        return false;
    return true;
}
function buildSearchFilter(t, orderColumn) {
    const pattern = `%${t}%`;
    if (FULL_UUID.test(t)) {
        return client_1.Prisma.sql `AND (
      t.id = ${t}::uuid
      OR wi.reference_id = ${t}::uuid
      OR ${client_1.Prisma.raw(orderColumn)} ILIKE ${pattern}
      OR EXISTS (
    ...
    return client_1.Prisma.sql `AND (
    ${client_1.Prisma.raw(orderColumn)} ILIKE ${pattern}
    OR EXISTS (
      SELECT 1 FROM documents d
       WHERE d.task_id = t.id
...
async function listContractCatalogSlots(tx, params) {
    const companyFilter = params.companyId
        ? client_1.Prisma.sql `AND wi.company_id = ${params.companyId}::uuid`
        : client_1.Prisma.empty;
    const completedFrom = params.createdFrom
        ? client_1.Prisma.sql `AND t.completed_at >= ${`${params.createdFrom}T00:00:00.000Z`}::timestamptz`
        : client_1.Prisma.empty;
    const completedTo = params.createdTo
        ? client_1.Prisma.sql `AND t.completed_at <= ${`${params.createdTo}T23:59:59.999Z`}::timestamptz`
        : client_1.Prisma.empty;
    const unions = [];
    if (includesReceiving(params.type, params.referenceType)) {
        const searchFilter = params.search?.trim()
            ...
            : client_1.Prisma.empty;
        unions.push(client_1.Prisma.sql `
      SELECT
        t.id AS task_id,
    ...
    const slotsCte = client_1.Prisma.sql `
    WITH slots AS (
      ${client_1.Prisma.join(unions, ' UNION ALL ')}
    ),
    enriched AS (
      SELECT
        s.*,
  ...
  `;
    ...
    if (params.pendingOnly) {
        filters.push(client_1.Prisma.sql `(en_id IS NULL OR ar_id IS NULL)`);
    }
    if (params.completeOnly) {
        filters.push(client_1.Prisma.sql `(en_id IS NOT NULL AND ar_id IS NOT NULL)`);
    }
    if (params.generatedOnly) {
        filters.push(client_1.Prisma.sql `(en_id IS NOT NULL OR ar_id IS NOT NULL)`);
    }
    if (params.missingLanguage === 'en') {
        filters.push(client_1.Prisma.sql `en_id IS NULL`);
    }
    if (params.missingLanguage === 'ar') {
        filters.push(client_1.Prisma.sql `ar_id IS NULL`);
    }
    if (params.language === 'en') {
        filters.push(client_1.Prisma.sql `en_id IS NOT NULL`);
    }
    if (params.language === 'ar') {
        filters.push(client_1.Prisma.sql `ar_id IS NOT NULL`);
    }
    const whereClause = filters.length > 0 ? client_1.Prisma.sql `WHERE ${client_1.Prisma.join(filters, ' AND ')}` : client_1.Prisma.empty;
    const countRows = await tx.$queryRaw(client_1.Prisma.sql `
    ${slotsCte}
    SELECT COUNT(*)::bigint AS total FROM enriched ${whereClause}
  `);
    const total = Number(countRows[0]?.total ?? 0);
    const rawRows = await tx.$queryRaw(client_1.Prisma.sql `
    ${slotsCte}
...
function mapRow(row) {
    const en = row.en_id && row.en_number && row.en_created
        ? {
        ...
        : null;
    const ar = row.ar_id && row.ar_number && row.ar_created
        ? {
...
//# sourceMappingURL=contracts-catalog.query.js.map