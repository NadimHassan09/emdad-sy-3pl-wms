...
function verifyScrypt(plain, stored) {
    ...
    if (parts.length !== 4 || parts[0] !== 'scrypt1')
        return false;
    const salt = Buffer.from(parts[2], 'base64url');
    const expected = Buffer.from(parts[3], 'base64url');
    const key = (0, node_crypto_1.scryptSync)(plain, salt, expected.length);
    return (0, node_crypto_1.timingSafeEqual)(key, expected);
}
