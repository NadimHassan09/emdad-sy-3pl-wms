...
exports.ledgerBusinessGroupKeySql = ledgerBusinessGroupKeySql;
exports.ledgerSignedQuantitySql = ledgerSignedQuantitySql;
exports.buildLedgerListSqlContext = buildLedgerListSqlContext;
exports.ledgerBusinessGroupsCountSql = ledgerBusinessGroupsCountSql;
exports.ledgerEntrySiblingRowsSql = ledgerEntrySiblingRowsSql;
...
function ledgerSignedQuantitySql(alias = 'il') {
    const a = client_1.Prisma.raw(alias);
    return client_1.Prisma.sql `
    CASE
...
async function buildLedgerListSqlContext(_prisma, companyAccess, user, query) {
    const companyId = (0, company_read_scope_1.readCompanyIdFilterRequired)(companyAccess, user, query.companyId);
    const joins = [
        client_1.Prisma.sql `INNER JOIN companies c ON c.id = il.company_id`,
        client_1.Prisma.sql `INNER JOIN products p ON p.id = il.product_id`,
    ...
    if (query.productId) {
        conditions.push(client_1.Prisma.sql `il.product_id = ${query.productId}::uuid`);
    }
    ...
    if (query.referenceType) {
        conditions.push(client_1.Prisma.sql `il.reference_type = ${query.referenceType}::ledger_ref_type`);
    }
    if (query.referenceId) {
        conditions.push(client_1.Prisma.sql `il.reference_id = ${query.referenceId}::uuid`);
    }
    if (query.createdFrom) {
        conditions.push(client_1.Prisma.sql `il.created_at >= ${new Date(`${query.createdFrom}T00:00:00.000Z`)}::timestamptz`);
    }
    if (query.createdTo) {
        conditions.push(client_1.Prisma.sql `il.created_at <= ${new Date(`${query.createdTo}T23:59:59.999Z`)}::timestamptz`);
    }
...
function ledgerBusinessGroupsCountSql(ctx) {
    return client_1.Prisma.sql `
...
const LEDGER_GROUP_PAGE_CTE = (ctx) => client_1.Prisma.sql `
  WITH filtered AS (
    SELECT
      il.id,
      il.created_at,
...
`;
function ledgerEntrySiblingRowsSql(input) {
...
function ledgerBusinessGroupPageSql(ctx, limit, offset) {
    return client_1.Prisma.sql `
...
//# sourceMappingURL=ledger-list.query.js.map