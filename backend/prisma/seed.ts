/**
 * Seed script — Phase 1 bootstrap (JWT auth).
 *
 * Inserts deterministic demo data so the UI is browseable end-to-end:
 *   - 5 client Companies (optional `X-Company-Id` / `VITE_MOCK_COMPANY_ID` for API tenant scope)
 *   - Internal users (`company_id` always null): `superadmin@emdad.example` (super_admin) and
 *     `manager@emdad.example` (wh_manager), password `demo123` (bcrypt hash below)
 *   - Client portal user (Acme Imports): `client@acme.example` (client_admin), password `demo123`
 *   - 1 Warehouse with two Locations
 *
 * IMPORTANT: All IDs are valid UUID v4 (version digit = 4) so they pass the
 * IsUuidLoose validator and any third-party UUID checks.
 *
...
async function main() {
  ...
  /** bcrypt hash for password `demo123` (internal demo user only). */
...
/** Drop user-created warehouse code WH (keep WH-001 Main Warehouse). */
