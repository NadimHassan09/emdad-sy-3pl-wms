...
export function InboundOrdersPage(): ReactElement {
  ...
  const { data, isLoading, isFetching, error } = useQuery({
    queryKey: ['client', 'inbound-orders', offset, PAGE_SIZE, appliedSearch],
    queryFn: () =>
  ...
  return (
    <main className="main">
      <div className="card">
        <h1 className="card__title">Inbound orders</h1>
        ...
        {error ? (
          <p className="banner banner--error" role="alert">
            Could not load inbound orders. Please try again.
          </p>
        ) : null}

        {isLoading ? (
          <p className="muted">Loading inbound orders…</p>
        ) : data && data.items.length === 0 ? (
          <p className="muted">No inbound orders found.</p>
        ) : data ? (
          <>
                    ...
                    <tr
                      ...
                      onKeyDown={(e) => {
                        if (e.key === 'Enter' || e.key === ' ') {
                          e.preventDefault();
                          navigate(`/inbound-orders/${row.id}`);
                        }
                      }}
                    >
                      <td>{row.orderNumber || row.id.slice(0, 8)}</td>
                      <td>{row.status}</td>
                      <td>{formatDate(row.expectedArrivalDate)}</td>
