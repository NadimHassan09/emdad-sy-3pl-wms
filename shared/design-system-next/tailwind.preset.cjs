...
module.exports = {
  ...
  theme: {
    extend: {
      colors: {
        /* Neutral ramp — light-theme slate */
        neutral: {
        ...
        /* Dark ramp — zinc-based near-black, used explicitly via `dark:` variants
           (deliberately NOT theme-swapped under the `neutral` key — a raw palette
           step means different things in each theme, so components opt in with
           `dark:bg-dark-800` etc. rather than the token silently flipping). */
        dark: {
          0:   'var(--color-dark-0)',
        ...
        /* Surface layer aliases — theme-aware via CSS vars, use these instead of
           the raw neutral/dark ramps whenever a value should flip with theme. */
        surface: {
        ...
        /* Status pill aliases — theme-aware tinted-badge language. */
        status: {
          ...
          'violet-fg': 'var(--status-violet-fg)',
          'violet-bg': 'var(--status-violet-bg)',
          'violet-border': 'var(--status-violet-border)',
        ...
        /* Brand "loudest accent" alias — resolves to different ramp steps per
           theme so a single class works everywhere (see tokens.css section 2). */
        cta: {
          DEFAULT: 'var(--brand-cta)',
          hover:   'var(--brand-cta-hover)',
          active:  'var(--brand-cta-active)',
        },
      ...
      fontSize: {
        ...
        base:   ['var(--text-base-size)', { lineHeight: 'var(--leading-normal)' }],
