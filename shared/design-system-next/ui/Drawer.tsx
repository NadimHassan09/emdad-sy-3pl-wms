    ...
    <Portal>
      <div className="fixed inset-0 z-drawer">
        ...
        <div
          ...
          tabIndex={-1}
          className={cn(
            'absolute inset-y-0 bg-surface-panel shadow-xl flex flex-col',
            anchor,
            SIZE[size],
            className,
          )}
        >
          {(title || !hideCloseButton) && (
            <div className="flex items-start gap-3 px-5 py-3 border-b border-border shrink-0">
              <div className="flex-1 min-w-0">
                {title && (
                  <h2 id={titleId} className="text-base font-semibold text-text-strong m-0 truncate">
                    {title}
                  </h2>
                )}
                {description && (
          ...
          <div className="flex-1 overflow-auto px-5 py-4 text-sm text-text-body">
            {children}
          </div>

          {footer && (
            <div className="flex flex-wrap items-center justify-end gap-2 px-5 py-3 border-t border-border bg-surface-card-muted shrink-0">
              {footer}
            </div>
          )}
        </div>
