...
const ROW_STATE_STYLES: Record<RowState, string> = {
  ...
  muted:   'bg-surface-sunken opacity-70',
  ...
  return (
    <th
      ...
      className={cn(
        ...
        ALIGN_CLASS[align],
        sticky && 'sticky start-0 z-raised bg-surface-card-muted after:absolute after:inset-y-0 after:end-0 after:w-px after:bg-border',
        sortable && 'cursor-pointer select-none hover:text-text-strong',
        className,
      )}
...
function SortArrow({ dir, active }: { dir: 'asc' | 'desc'; active?: boolean }) {
  return (
    <svg
      width="8" height="5"
      viewBox="0 0 8 5"
      className={cn('block', active ? 'text-brand-600 dark:text-brand-400' : 'text-text-faint')}
      fill="currentColor"
    >
      {dir === 'asc'
...
function Td({
  ...
  return (
    <td
      className={cn(
        'align-middle',
        numeric ? 'font-mono text-text-strong' : 'text-text-body',
        ALIGN_CLASS[numeric ? 'end' : align],
        sticky && 'sticky start-0 z-raised bg-inherit after:absolute after:inset-y-0 after:end-0 after:w-px after:bg-border',
        className,
      )}
      {...rest}
    >
...
export function DataTable<T>({
  ...
  return (
    <div
      className={cn(
        'w-full overflow-hidden rounded-xl border border-border bg-surface-card shadow-sm',
        className,
      )}
    >
                ...
                return (
                  <tr
                    ...
                    className={cn(
                      ...
                      isClickable &&
                        'cursor-pointer transition-[background-color] duration-fast ease-standard hover:bg-surface-hover focus-visible:outline-none focus-visible:bg-surface-hover',
                      /* Zebra stripe — only on rows without an explicit state */
                      zebra && !hasState && index % 2 === 1 && 'bg-surface-card-muted',
...
export function DataTableContainer({
  ...
  return (
    <div
      className={cn(
        'flex flex-col gap-0 overflow-hidden rounded-xl border border-border bg-surface-card shadow-sm',
        className,
      )}
      {...rest}
    >
