...
function NotificationsDropdown({
  ...
  return createPortal(
    <>
      ...
      >
        <div className="flex items-center justify-between gap-2 border-b border-border-subtle bg-surface-card-muted px-4 py-3">
        ...
        <div className="max-h-[min(24rem,70vh)] overflow-y-auto">
          {loading && items.length === 0 ? (
            <p className="px-4 py-8 text-center text-sm text-text-muted">…</p>
          ) : items.length === 0 ? (
                ...
                <li key={item.id}>
                  <button
                    ...
                    className={cn(
                      ...
                      !item.isRead && 'bg-brand-50/40 dark:bg-white/[0.03]',
                    )}
                    onClick={() => {
                      onItemClick?.(item);
                      onClose();
                  ...
                  >
                    ...
                    <p className="text-xs leading-relaxed text-text-muted line-clamp-2">{item.body}</p>
                  </button>
                </li>
              ))}
            </ul>
        ...
        {(viewAllHref || onViewAll) && viewAllLabel ? (
          <div className="border-t border-border-subtle bg-surface-card-muted px-4 py-2.5 text-center">
            {viewAllHref ? (
              <a
                href={viewAllHref}
