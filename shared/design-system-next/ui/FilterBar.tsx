  ...
  return (
    <button
      ...
      className={cn(
        ...
        open
          ? 'border-brand-600 bg-brand-50 text-brand-700 dark:bg-white/5 dark:text-brand-400'
          : 'border-border-strong bg-surface-panel text-text-body hover:bg-surface-hover',
      )}
...
export function FilterBarActions({
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 pt-3 border-t border-border mt-1">
      <Button
        type="button"
        variant="primary"
        size="sm"
      ...
      {loading && <Spinner size="sm" className="text-text-faint" />}
    </div>
  );
}
  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <select
        ...
        disabled={disabled}
        className={cn(
          'h-8 w-full bg-surface-panel border border-border-strong rounded-lg text-sm',
          'ps-3 pe-9 text-sm text-text-strong',
          'appearance-none',
