  ...
  return (
    ...
    >
      {(slots) => (
        <div
          className={cn(
            'flex items-center bg-surface-panel border rounded-lg overflow-hidden',
            'transition-colors duration-fast ease-standard',
            error
              ? 'border-danger-400 focus-within:border-danger-500 focus-within:shadow-focus-danger'
              : 'border-border-strong focus-within:border-brand-500 focus-within:shadow-focus',
            disabled && 'bg-surface-sunken opacity-70 cursor-not-allowed',
          )}
          style={{ borderRadius: 'var(--radius-input)' }}
        >
          {startAdornment && (
            <span className="ps-3 pe-1 text-text-muted shrink-0 flex items-center" aria-hidden="true">
              {startAdornment}
            </span>
          )}
          <input
          ...
          />
          {endAdornment && (
            <span className="pe-3 ps-1 text-text-muted shrink-0 flex items-center" aria-hidden="true">
              {endAdornment}
            </span>
          )}
        </div>
