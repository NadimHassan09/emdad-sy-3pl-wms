...
export function TableToolbar({
  ...
  return (
    <div className={cn('border-b border-border-subtle bg-surface-panel', className)}>
      {/* Header row */}
      <div className={cn('flex flex-wrap items-center gap-3', px, py)}>
        {/* Title */}
        {title && (
          <h2 className="m-0 me-auto whitespace-nowrap text-base font-semibold text-text-strong sm:text-lg">
            {title}
          </h2>
        )}
      ...
      {/* Filter row */}
      {filters && (
        <div className={cn('border-t border-border-subtle bg-surface-card-muted', px, 'py-3')}>
          {filters}
        </div>
      )}
    </div>
...
export function DensityToggle({ density, onChange }: DensityToggleProps) {
  return (
    <div
      ...
      className="flex items-center rounded-lg border border-border overflow-hidden bg-surface-panel"
