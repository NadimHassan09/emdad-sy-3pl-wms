...
export function SearchInput({
  ...
  return (
    ...
    >
      <div
        className={cn(
          'flex flex-1 items-center rounded-lg border bg-surface-panel',
          'transition-colors duration-fast ease-standard',
          'focus-within:border-brand-500 focus-within:shadow-focus',
          disabled ? 'border-border-subtle bg-surface-sunken opacity-70' : 'border-border-strong',
        )}
      ...
      >
        ...
        <span
          aria-hidden="true"
          className="ps-3 text-text-faint shrink-0 flex items-center"
        >
          {isLoading ? (
            <Spinner size="sm" />
          ) : (
        ...
        {value && !disabled && (
          <button
            ...
            className={cn(
              'pe-2.5 text-text-faint hover:text-text-body',
              'flex items-center shrink-0 transition-colors duration-fast',
              'focus-visible:outline-none focus-visible:text-text-strong',
            )}
          >
