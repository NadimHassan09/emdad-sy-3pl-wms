...
export function TableCardHeader({
  ...
  return (
    <div
      className={cn(
        'flex flex-col gap-2 border-b border-border-subtle px-3 py-3 sm:px-4 md:flex-row md:items-start md:justify-between',
        className,
      )}
      {...rest}
    >
      {(title || description) && (
        <div className="min-w-0">
          {title && (
            <TitleTag className="text-base font-bold text-text-strong sm:text-lg">{title}</TitleTag>
          )}
          {description && (
